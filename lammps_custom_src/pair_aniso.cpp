...
/* New potential types taken from pair_cosine_squared.cpp */
double PairAniso::cosine_sq_E(const double r, const double E, const double sigma, const double rcut){
  if (r < sigma){
    return -E;
  }else if ( r < rcut ){
    double cosone = cos(MathConst::MY_PI*(r-sigma)/(2.0*(rcut-sigma)));
    return -E*cosone*cosone;
  }else{
    return 0.0;
  }
}
double PairAniso::dcosine_sq_dr(const double r, const double E, const double sigma, const double rcut){
  if (r < sigma){
    return 0;
  }else if ( r < rcut ){
    double w = rcut-sigma;
    double forcecos = -(MathConst::MY_PI*E/(2.0*w)*sin(MathConst::MY_PI*(r-sigma)/w));
    return forcecos;
  }else{
    return 0.0;
  }
...
PairAniso::PairAniso(LAMMPS *lmp) : Pair(lmp)
{
...
void PairAniso::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        #endif
          //TODO test this case:
          printf("Warning untest SPHERE_SPHERE case in pair_aniso.cpp\n");
          //r2inv = 1.0/rsq;
          //r6inv = r2inv*r2inv*r2inv;
          //forcelj = r6inv * (lj1[itype][jtype]*r6inv - lj2[itype][jtype]);
          //forcelj *= -r2inv;
          double r = sqrt(rsq);
          forcelj = -dcosine_sq_dr(r,epsilon[type[i]][type[j]],sigma[type[i]][type[j]],mycut[type[i]][type[j]])/r;
          //if (eflag) one_eng =
          //             r6inv*(r6inv*lj3[itype][jtype]-lj4[itype][jtype]) -
          //             offset[itype][jtype];
          if (eflag) one_eng = cosine_sq_E(r,epsilon[type[i]][type[j]],sigma[type[i]][type[j]],mycut[type[i]][type[j]]);
        ...
        default:
          ...
          printf("Warning untest ELLIPSE_ELLIPSE case in pair_aniso.cpp\n");
...
void PairAniso::allocate()
{
  ...
  memory->create(mycut,n+1,n+1,"pair:mycut");
...
void PairAniso::settings(int narg, char **arg)
{
...
void PairAniso::coeff(int narg, char **arg)
{
  if (narg < 11 || narg > 12)
...
double PairAniso::init_one(int i, int j)
{
  ...
  if (setflag[i][j] == 0) {
    ...
    mycut[i][j] = mix_distance(mycut[i][i],mycut[j][j]);
  }
...
void PairAniso::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++) {
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        ...
        fwrite(&mycut[i][j],sizeof(double),1,fp);
      }
    }
  }
}
...
void PairAniso::read_restart(FILE *fp)
{
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          ...
          utils::sfread(FLERR,&mycut[i][j],sizeof(double),1,fp,NULL,error);
        }
        MPI_Bcast(&epsilon[i][j],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&sigma[i][j],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&cut[i][j],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&mycut[i][j],1,MPI_DOUBLE,0,world);
      }
    }
  }
}
...
void PairAniso::write_restart_settings(FILE *fp)
{
...
void PairAniso::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairAniso::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void PairAniso::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairAniso::gayberne_analytic(const int i,const int j,double a1[3][3],
                                       ...
                                       double *ttor, double *rtor)
{
  ...
  // energy
  // compute u_r
  double varrho = h12 + gamma*sigma[type[i]][type[j]];
  ...
  double u_r = cosine_sq_E(varrho,epsilon[type[i]][type[j]],sigma[type[i]][type[j]],mycut[type[i]][type[j]]);
  ...
  temp1 = dcosine_sq_dr(varrho,epsilon[type[i]][type[j]],sigma[type[i]][type[j]],mycut[type[i]][type[j]]);
...
double PairAniso::gayberne_lj(const int i,const int j,double a1[3][3],
...
void PairAniso::compute_eta_torque(double m[3][3], double m2[3][3],
