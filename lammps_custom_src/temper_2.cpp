...
// #define Temper22_DEBUG 1

/* ---------------------------------------------------------------------- */

Temper2::Temper2(LAMMPS *lmp) : Command(lmp) {}
...
/* ----------------------------------------------------------------------
   perform Temper2ing with inter-world swaps
------------------------------------------------------------------------- */

void Temper2::command(int narg, char **arg)
{
  if (universe->nworlds == 1)
    error->all(FLERR,"Must have more than one processor partition to Temper2");
  if (domain->box_exist == 0)
    error->all(FLERR,"Temper2 command before simulation box is defined");
  if (narg != 7 && narg != 8)
    error->universe_all(FLERR,"Illegal Temper2 command");
  ...
  // ignore Temper2 command, if walltime limit was already reached
...
for (whichfix2 = 0; whichfix2 < modify->nfix; whichfix2++)
  ...
  if ((my_set_temp < 0) || (my_set_temp >= universe->nworlds))
    error->universe_one(FLERR,"Illegal Temper2ature index");

  // swap frequency must evenly divide total # of timesteps

  if (nevery <= 0)
    error->universe_all(FLERR,"Invalid frequency in Temper2 command");
  nswaps = nsteps/nevery;
  if (nswaps*nevery != nsteps)
    error->universe_all(FLERR,"Non integer # of swaps in Temper2 command");

  // fix style must be appropriate for Temper2ature control, i.e. it needs
  ...
  if ((!utils::strmatch(modify->fix[whichfix]->style,"^nvt")) &&
    ...
    error->universe_all(FLERR,"Temper2ing Temperature fix is not supported");
 ...
 if ((!utils::strmatch(modify->fix[whichfix2]->style,"^nvt")) &&
  ...
  if (id < 0) error->all(FLERR,"Temper2ing could not find thermo_pe compute");
  ...
  // create static list of set Temper2atures
  // allgather Temper2ing arg "temp" across root procs
  ...
  // if restarting Temper2ing, reset temp target of Fix to current my_set_temp
...
void Temper2::scale_velocities(int t_partner, int t_me)
{
...
/* ----------------------------------------------------------------------
   proc 0 prints current Temper2ing status
------------------------------------------------------------------------- */

void Temper2::print_status()
{
  if (universe->uscreen) {
